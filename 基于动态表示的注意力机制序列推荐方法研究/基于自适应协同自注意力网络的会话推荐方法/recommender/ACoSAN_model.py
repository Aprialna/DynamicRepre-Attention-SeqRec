...
beta = 0.3
gama = 0.3
class ACoSAN_model(TrainEvaluate):

    def __init__(self, dataModel, config):

        super(ACoSAN_model, self).__init__(dataModel, config)
        ...
        self.train_last_item = dataModel.train_last_item
        self.train_last_nei = dataModel.train_last_nei
        self.train_last_weight = dataModel.train_last_weight
        ...
        self.pred_target_item = dataModel.test_target_item
        self.pred_target_nei = dataModel.test_target_nei
        self.pred_target_weight = dataModel.test_target_weight
        ...
        self.last_item = tf.placeholder(tf.int32, [self.trainBatchSize, 1], name='last_item')
        self.last_nei = tf.placeholder(tf.int32, [self.trainBatchSize, self.familiar_user_num], name='last_nei')
        self.last_weight = tf.placeholder(tf.float32, [self.trainBatchSize, self.familiar_user_num], name='last_weight')
        # test placeholder
        ...
        self.test_target_item = tf.placeholder(tf.int32, [self.testBatchSize, 1], name='test_target_item')
        self.test_target_nei = tf.placeholder(tf.int32, [self.testBatchSize, self.familiar_user_num],
                                              name='test_target_nei')
        self.test_target_weight = tf.placeholder(tf.float32, [self.testBatchSize, self.familiar_user_num],
                                                 name='test_target_weight')
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            # last embedding
            last_item_embedding = tf.nn.embedding_lookup(self.C_itemEmbedding, self.last_item)
            last_nei_embedding = tf.nn.embedding_lookup(self.C_userEmbedding, self.last_nei)
            target_item_embedding = tf.nn.embedding_lookup(self.C_itemEmbedding, self.test_target_item)
            target_nei_embedding = tf.nn.embedding_lookup(self.C_userEmbedding, self.test_target_nei)
            ...
            rating_loss = self.train_pred(user_embedding_drop, C_familiar_user_embedding_drop, C_memory,
                                            ...
                                            last_item_embedding, last_nei_embedding, self.last_weight)
            ...
            self.r_pred = self.test_pred(userEmbedding_test, C_familiar_user_embedding_test,
                                                 ...
                                                 target_item_embedding, target_nei_embedding, self.test_target_weight)


    def train_pred(self, user_embedding_drop, C_familiar_user_embedding_drop, C_memory, target_item_ids,
                     negative_item_ids, weight, last_item_embedding, last_neighbor_embedding, last_weight):
        ...
        item_embed_input = tf.nn.dropout(tf.reshape(C_item_pre_embedding, [-1, (self.input_length-1), self.numFactor]),
                                         self.keep_prob_user)
        # 计算最后一个item的collaborative item embedding
        last_feature = tf.reshape(tf.reduce_sum(tf.multiply(
            last_neighbor_embedding, tf.expand_dims(last_weight, axis=-1)), axis=1),
            [-1, 1, self.numFactor])
        last_colla = tf.add(last_item_embedding, tf.multiply(tf.clip_by_value(self.userC_weight, 0.1, 1.0),
                                                             last_feature))

        last_colla_embedding = tf.reshape(last_colla, [-1, 1, self.numFactor])
        adap_weight = tf.nn.softmax(tf.matmul(item_embed_input, tf.transpose(last_colla_embedding, [0, 2, 1])))
        adap_input = tf.multiply(adap_weight, item_embed_input)
        for i in range(self.mlpnum):
            w = tf.Variable(tf.random_normal([self.numFactor, self.numFactor]))
            b = tf.Variable(tf.random_normal([self.numFactor]))
            adap_input = tf.nn.xw_plus_b(adap_input, w, b)
        adap_user = tf.nn.softmax(adap_input)
        ...
        # SAN, multiply & add
        user_embedding_drop = alpha*self.seq + beta*tf.expand_dims(user_embedding_drop, axis=1)+gama*adap_user
    ...
    def test_pred(self, user_embedding_test, C_familiar_user_embedding_test, C_memory_test, target_item_test,
                          weight, last_item_embedding, last_neighbor_embedding, last_weight):
        ...
        user_embedding_test = alpha*self.seq + beta*tf.expand_dims(user_embedding_test, axis=1)+gama*adap_user
    ...
    def getTrainData(self, batchId):
        ...
        last_item_batch = self.train_last_item[start_idx:end_idx]
        last_nei_batch = self.train_last_nei[start_idx:end_idx]
        last_weight_batch = self.train_last_weight[start_idx:end_idx]
        ...
        input_user_seq_batch = np.array(input_user_seq_batch).reshape(-1,
                                                                      (self.input_length - 1) * self.familiar_user_num)
        input_user_seq_weight_batch = np.array(input_user_seq_weight_batch).reshape(-1, (
                self.input_length - 1) * self.familiar_user_num)
        pos_seq_batch = np.array(pos_seq_batch)
        neg_seq_batch = np.array(neg_seq_batch)
        last_item_batch = np.array(last_item_batch)
        last_nei_batch = np.array(last_nei_batch).reshape(-1, self.familiar_user_num)
        last_weight_batch = np.array(last_weight_batch).reshape(-1, self.familiar_user_num)
        ...
        feed_dict = {
            ...
            self.last_item: last_item_batch,
            self.last_nei: last_nei_batch,
            self.last_weight: last_weight_batch,
    ...
    def getPredList_ByUserIdxList(self, user_idices):
        ...
        for userIdx in user_idices:
            ...
            target_item.append(self.pred_target_item[userIdx])
            target_nei.append(self.pred_target_nei[userIdx])
            target_weight.append(self.pred_target_weight[userIdx])
        ...
        input_user_seq_weight = np.array(input_user_seq_weight).reshape(
            (-1, (self.input_length - 1) * self.familiar_user_num))
        target_seq = np.array(target_seq)
        target_item = np.array(target_item)
        target_nei = np.array(target_nei)
        ...
        predList = self.sess.run(self.r_pred, feed_dict={
            ...
            self.test_target_item: target_item,
            self.test_target_nei: target_nei,
            self.test_target_weight: target_weight,
